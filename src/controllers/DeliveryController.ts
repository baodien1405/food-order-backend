...
import { CreateDeliveryUserInputs, EditDeliveryUserProfileInputs, UserLoginInputs } from '../dto'
...
export const EditDeliveryUserProfile = async (req: Request, res: Response, next: NextFunction) => {
  const deliveryUser = req.user
  const profileInputs = plainToInstance(EditDeliveryUserProfileInputs, req.body)
